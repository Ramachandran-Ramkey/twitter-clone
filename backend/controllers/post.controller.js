...
export const createComment = async (req , res) => {
    try{
        ...
        if(!text) {
            return res.status(400).json({error : "Comment text is requried"})
        }
...
export const likeUnlikePost = async (req , res) => {
    try {
        ...
        if(userLikedPost) {
            ...
            await User.updateOne({_id : userId}, {$pull : {likeedPosts : postId}})
            
            const updatedLikes = post.likes.filter((id) => id.toString() !== userId.toSting())
